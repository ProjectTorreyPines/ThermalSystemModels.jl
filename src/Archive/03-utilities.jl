...
function vcat_dict!(dictA::Dict,dictB::Dict)
    # only works for graph edges
    source_node = [keyval[1] for keyval ∈ collect(keys(dictB))]
    target_node = [keyval[2] for keyval ∈ collect(keys(dictB))]

    shift_amount = length(dictA)

    new_src = source_node .+ shift_amount
    new_trg = target_node .+ shift_amount

    for i = 1:length(dictB)
        dictA[(new_src[i], new_trg[i])] = dictB[(source_node[i],target_node[i])]
    end

end

function vcat_dic!(dictA::Dict,dictB::Dict)
    # sketchy naming here
    # this just working for 1d dicts

    source_node = [keyval for keyval ∈ collect(keys(dictB))]
    ...
    for i = 1:length(dictB)
        dictA[(new_src[i])] = dictB[(source_node[i])]
    end
