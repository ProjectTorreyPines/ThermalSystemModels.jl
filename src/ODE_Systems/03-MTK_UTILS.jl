...
function showsol(c,sol::ODESolution)
    ...
    for cel in c
        ...
        for n in cvec
            @printf "%-13s  P[bar]= %-8.1f T[K]= %-8.1f ṁ[kg/s]= %+-7.2f Φ=%+-7.2f" n.name sol[n.P][end] sol[n.T][end] sol[n.ṁ][end] sol[n.Φ][end]/10^6
            if hasproperty(n,:h)
...
function showsol(c,prob::ODEProblem; prob_attr = nothing)
    ...
    for cel in c
        ...
        for n in cvec
            ...
            push!(evars,n.Φ)
            push!(evars,n.ṁ)
            @printf "%-13s  P[bar]= %-8.1f T[K]= %-8.1f ṁ[kg/s]= %+-7.2f Φ=%+-8.2f" sname sol(n.P) sol(n.T) sol(n.ṁ) sol(n.Φ)/10^6
            if hasproperty(n,:h)
                ...
                for s in src
                    for d in dst
                        ...
                        if portNames[s] != portNames[d]
                            add_edge!(g,component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])])
                            cp = cname[d]
                            cp[end] = "Φ"
                            pwr_id = Symbolics.tosymbol(join(cp,"₊"); escape = false)
                            edge_power_dict[(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])])] = Symbol(pwr_id)
...
function system2graph2(ODESYS::ODESystem, utility_vector::Vector{Symbol}; verbose = false,soln = nothing)
    ...
    for eq in eqs
        ...
        elseif check_for_Q(symvar)
            ...
            if length(non_util) != length(portNames)
                ...
                util_src = true
                for flg in pflags
                    verbose ? println("  $(flg) => $(util_component) $( occursin(lowercase(String(flg)),lowercase(String(util_component))))") : nothing
                ...
                for (i,port) in enumerate(non_util)
                    ...
                    edge_power_dict[(component_name_dict[src], component_name_dict[dst])] = Symbolics.tosymbol(svar)
                end
        ...
        elseif check_for_Ẇ(symvar) 
            ...
            if length(non_util) != length(portNames)
                ...
                for (i,port) in enumerate(non_util)
                    ...
                    add_edge!(g,component_name_dict[src], component_name_dict[dst]) 
                    edge_power_dict[(component_name_dict[src], component_name_dict[dst])] = Symbolics.tosymbol(symvar[idx])
                end
            end
        end
    end
...
function add_graph_connection!(sys, g, component_dict, vertex_dict, Component_Variable,edge_power_dict; flip_dir = false, verbose = false)
    ...
    for eq in eqs_to_search
        ...
        for (i,ni) in enumerate(non_input)
            ...
            #component -> reservoir
            if occursin("turb", ni) || occursin("conde", ni) || occursin("Rej", ni) || flip_dir == true
            ...
            if src !=  dst 
                ...
                if verbose
                    ...
                    println("    var $(vertex_dict[component_dict[src]]) -> $(vertex_dict[component_dict[dst]])")
                end
...
function system2metagraph(sys::ODESystem, utility_vector::Vector{Symbol}; verbose = false,soln = nothing)
                ...
                for s in src
                    for d in dst
                        ...
                        if portNames[s] != portNames[d]
                            ...
                            dsys = get_prop(G,component_name_dict[Symbol(portNames[d])],:sys)
                            verbose ? println("$(Symbol(portNames[s])) => $(dsys.name)") : nothing
                            # verbose ? println("$(dsys.name) $(typeof(portid[d]))") : nothing
                            set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :etype, :flow)
                            if portid[d] == "p"
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :evar,  dsys.p.Φ)
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :mflow, dsys.p.ṁ)
                            elseif portid[d] == "p1"
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :evar,  dsys.p1.Φ)
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :mflow, dsys.p1.ṁ)
                            elseif portid[d] == "p2"
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :evar,  dsys.p2.Φ)
                                set_prop!(G,Edge(component_name_dict[Symbol(portNames[s])], component_name_dict[Symbol(portNames[d])]), :mflow, dsys.p2.ṁ)
                            else
                                println("Failed to find destination node name for $(Symbol(portNames[s])) => $(dsys.name) , add key $(portid[d])")
                            end
                        end
                    end
                end
        ...
        elseif check_for_Q(symvar)
            ...
            if length(non_util) != length(portNames)
                ...
                for (i,port) in enumerate(non_util)
                    ...
                    svar = vars[idx]
                    add_edge!(G,component_name_dict[src], component_name_dict[dst]) 
                    set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:etype,etyp)
                    set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:evar,svar)
                end
            ...
            elseif length(portNames) == 2
                ...
                add_edge!(G,component_name_dict[src], component_name_dict[dst]) 
                set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:etype,:transferheat)
                set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:evar,svar)
            end
        ...
        elseif check_for_Ẇ(symvar) 
            ...
            if length(non_util) != length(portNames)
                ...
                for (i,port) in enumerate(non_util)
                    ...
                    add_edge!(G,component_name_dict[src], component_name_dict[dst]) 
                    set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:etype,:electric)
                    set_prop!(G,Edge(component_name_dict[src], component_name_dict[dst]),:evar,vars[idx])
                end
            end
        end
    end
