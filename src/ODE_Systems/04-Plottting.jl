...
function minimize_edge_crossings(
    graph,
    start_layout;
    NITER = 100,
    K = 1.5,
    ...
    for i = 1:NITER
        ...
        for edge1 in edges(graph)
            ...
            for edge2 in edges(graph)
                ...
                if bool
                    ...
                    current_layout[edge1_node] =
                        current_layout[edge2_node] + K .* (rand(2) .- 0.5)
        ...
        verbose ? println("     Iter: $(i) , crossings = $(crossings), Δ = $(chag)") :
        nothing
...
function minimize_edge_crossings_shell(
    graph,
    inipos,
    nlist;
    scaling = 1.0,
    ...
    for i = 1:NITER
        for lev in nlist
            ...
            for nprm in num_perms
                ...
                if crossings < best_crossings
                    verbose ?
                    println("ITER $(i) reduced $(best_crossings) to $(crossings)") : nothing
...
function minimize_edge_crossings5(
    graph,
    start_layout;
    NITER = 100,
    K = 1.5,
    ...
    for i = 1:NITER
        ...
        if full_check == true
            ...
            if f_crossings < best_full_crossings && (sys_crossings < best_sys_crossings)
                println(
                    "ITER $(i) selected reduced $(best_sys_crossings) to $(sys_crossings)",
                )
                println(
                    "      full level reduction $(best_full_crossings) to $(f_crossings)",
                )
        ...
        for nl in node_list
            ...
            for i = 1:niter2
                ...
                if new_sys_crossings < best_full_crossings
                    println(
                        "ITER $(i) System Intersections Reduced $(best_full_crossings) to $(new_sys_crossings)",
                    )
...
function rotate(
    c::Vector{Point2{Float32}},
    ang::Vector{Float64};
    scale_factor = 1.0,
    translation = Point2(0.0, 0.0),
)
    return [
        rotate(c[i], ang[i]; scale_factor = scale_factor, translation = translation) for
        i = 1:length(c)
    ]
end
...
function align_nodes(graph, positions)
    ...
    for edge in edges(graph)
        ...
        if dx != 0 && dy != 0
            if abs(dx) > abs(dy)
                # Align horizontally
                aligned_positions[dest] =
                    Point2f(aligned_positions[dest][1], aligned_positions[src][2])
                # aligned_positions[dest][2] = positions[src][2]
            else
                # # Align vertically
                # aligned_positions[dest][1] = positions[src][1]
                aligned_positions[dest] =
                    Point2f(aligned_positions[src][1], aligned_positions[dest][2])
            end
        end
    end
